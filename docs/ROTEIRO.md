...
3. Arquitetura: React comunica-se com uma API Python. A leitura usa texto do PDF ou visão multimodal. O Gemini estrutura dados, a validação confere evidências e o SQLite preserva o histórico.
...
Configurar `GEMINI_API_KEY` em `.env`, reiniciar a API e regravar a demonstração incluindo um upload de PDF e um de imagem. Aguardar a extração, abrir evidências e comparar dois documentos processados pelo modelo. Citar o modelo usado e informar limitações observadas.
